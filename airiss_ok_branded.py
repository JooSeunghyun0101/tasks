# AIRISS 완벽한 엔터프라이즈급 AI 분석 시스템 - OK금융그룹 완전 브랜딩
# OK금융그룹 CI 가이드라인 완벽 적용 + OK체 폰트 적용
# 파일명: airiss_ok_branded.py
...
# FastAPI 앱 초기화
app = FastAPI(
    title="AIRISS OK금융그룹 Enterprise AI System",
    description="OK금융그룹 CI 가이드라인을 완벽 적용한 AI 기반 직원 성과/역량 종합 스코어링 시스템",
    version="4.0.0"
)

# 정적 파일 서빙 설정 (OK체 폰트 포함)
try:
...
# AIRISS 8대 영역 완전 설계 (OK브랜딩 색상 적용)
AIRISS_FRAMEWORK = {
    "업무성과": {
        "keywords": {
            "positive": [
    ...
    "조직적응": {
        ...
        "color": "#9E9E9E",  # OK Bright Grey 변형
        "icon": "🏢"
    }
}
    ...
    async def generate_ai_feedback(self, uid: str, opinion: str, api_key: str = None, model: str = "gpt-3.5-turbo", max_tokens: int = 1200) -> Dict[str, Any]:
        ...
        if not api_key or api_key.strip() == "":
            return {
                ...
                "ai_weaknesses": "API 키를 입력하면 상세한 분석을 받을 수 있습니다.",
        ...
        try:
            ...
            prompt = f"""
OK금융그룹 직원 {uid}의 평가 의견을 AIRISS 8대 영역을 기반으로 분석해주세요.
...
[종합 피드백]
OK금융그룹 인재상에 맞는 구체적 피드백 500자 내외로 작성.
            """
            
            response = client.chat.completions.create(
                model=model,
                messages=[
                    {"role": "system", "content": "당신은 OK금융그룹의 전문 HR 분석가입니다."},
            ...
            return {
                ...
                "ai_feedback": f"OpenAI API 호출 중 오류 발생: {error_msg}",
                "processing_time": round(processing_time, 2),
                "model_used": model,
                "tokens_used": 0,
                "error": error_msg
    ...
    def parse_ai_response(self, response: str) -> tuple:
        ...
        try:
            ...
            if not strengths.strip():
                strengths = "분석 결과 다양한 긍정적 특성이 관찰됩니다."
            if not weaknesses.strip():
                weaknesses = "전반적으로 우수하나 지속적 성장 기회가 있습니다."
...
# OK금융그룹 브랜딩 완전 적용 HTML 템플릿
@app.get("/", response_class=HTMLResponse)
async def get_main_page():
    """메인 페이지 - OK금융그룹 완전 브랜딩"""
    html_content = """
<!DOCTYPE html>
<html lang="ko">
<head>
    ...
    <title>AIRISS | OK금융그룹 AI 인재분석시스템</title>
    ...
    <style>
        ...
        @font-face {
            font-family: 'OK';
            src: url('/static/fonts/OKBold.woff2') format('woff2'),
                 url('/static/fonts/OKBold.woff') format('woff'),
                 url('/static/fonts/OKBold.ttf') format('truetype');
            font-weight: 700;
            font-style: normal;
            font-display: swap;
        }
        ...
        @font-face {
            font-family: 'OK';
            src: url('/static/fonts/OKLight.woff2') format('woff2'),
                 url('/static/fonts/OKLight.woff') format('woff'),
                 url('/static/fonts/OKLight.ttf') format('truetype');
            font-weight: 300;
            font-style: normal;
            font-display: swap;
        }
...
<body>
    <div class="container">
        ...
        <div class="header">
            ...
            <h1>AIRISS 인재분석시스템</h1>
            <div class="subtitle">AI-Powered Employee Performance & Competency Analysis System</div>
            <div class="status-badge">
                <i class="fas fa-check-circle"></i> Enterprise Ready • Version 4.0 • OK금융그룹 완전 브랜딩
            </div>
        </div>
    ...
    <script>
            ...
            document.getElementById('uploadResult').innerHTML = `
                <div class="result-card">
                    ...
                    <div style="margin-top: 15px;">
                        ...
                        ${data.airiss_ready ? 
                            '<span style="color: #4CAF50;"><i class="fas fa-check"></i> 완전 준비됨</span>' : 
        ...
        function displayAnalysisResult(status) {
            document.getElementById('analysisResult').innerHTML = `
                <div class="result-card">
                    ...
                    <p style="text-align: center; margin-top: 20px; color: var(--ok-text-light);">
        ...
        document.addEventListener('DOMContentLoaded', function() {
            setupFileUpload();
            addLog('🎯 OK금융그룹 AIRISS 시스템 초기화 완료');
...
# 나머지 API 엔드포인트들은 이전과 동일하므로 생략...
# (upload, analyze, status, download 등의 함수들)
...
async def process_analysis(job_id: str):
    ...
    try:
        ...
        logger.info(f"분석 처리 시작: 샘플={sample_size}, AI={enable_ai}")
        ...
        for idx, row in sample_df.iterrows():
            try:
                ...
                result_record = {
                    ...
                    "OK등급": overall_analysis["grade"],  # OK★★★, OK★★, OK★ 등
                ...
                result_record["분석시스템"] = "AIRISS v4.0 - OK금융그룹 브랜딩"
        ...
        logger.info(f"분석 완료: {job_id}, 성공: {len(results)}, 실패: {job_data['failed']}")
...
async def create_excel_report(job_id: str, results: List[Dict], enable_ai: bool = False):
    ...
    try:
        ...
        result_path = f'results/OK금융그룹_AIRISS{ai_suffix}_{timestamp}.xlsx'
        ...
        logger.info(f"OK브랜딩 Excel 보고서 생성 완료: {result_path}")
...
async def download_results(job_id: str):
    ...
    filename = f"OK금융그룹_AIRISS_{ai_suffix}_{timestamp}.xlsx"
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("5. OpenAI 모듈 설치: pip install openai")
        input("엔터를 눌러 종료...")